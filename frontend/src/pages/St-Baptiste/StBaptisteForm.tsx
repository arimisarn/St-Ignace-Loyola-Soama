...
const StBaptisteForm = () => {
  ...
  const [nextCode, setNextCode] = useState<string>("BAPT001");
  ...
  useEffect(() => {
    axios
      .get("http://127.0.0.1:8000/st-baptiste/api/membres/")
      .then((res) => {
        const membres = res.data;
        const codes = membres.map((m: any) => m.code_kristianina);
        const numbers = codes.map((c: string) =>
          parseInt(c.replace("BAPT", ""), 10)
        );
        const maxNumber = numbers.length ? Math.max(...numbers) : 0;
        const newCode = `BAPT${(maxNumber + 1).toString().padStart(3, "0")}`;
        setNextCode(newCode);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await axios.post("http://127.0.0.1:8000/st-baptiste/api/membres/", payload);

      toast.success("Membre ajouté avec succès !");

      // Reset form sauf code kristianina
      const nextNumber = parseInt(nextCode.slice(3)) + 1;
      const newCode = `BAPT${nextNumber.toString().padStart(3, "0")}`;
      setNextCode(newCode);
        ...
        <div className="bg-white rounded-3xl shadow-xl border border-gray-100 overflow-hidden">
          <form onSubmit={handleSubmit} className="p-8 space-y-12">
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-4">
                <h3 className="text-lg font-semibold text-gray-800">
                  Fikambanana Masina / Vaomieran'Asa
                </h3>
                <div className="flex space-x-3">
                  <input
                    type="text"
